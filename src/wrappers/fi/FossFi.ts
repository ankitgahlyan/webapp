...
export function fossFiConfigToCell(config: FossFiConfig): Cell {
	...
	return beginCell()
        ...
        .storeBit(false)
		.storeRef(config.baseFiWalletCode)
		.storeRef(content)
...
export class FossFi implements Contract {
    ...
    */
    ...
    async getjettonDataAll(provider: ContractProvider) {
        let res = await provider.get('get_jetton_data_all', []); // todo: exitcode9
        let supply = res.stack.readBigNumber();
        let walletVersion = res.stack.readBigNumber();
        let admin = res.stack.readAddress();
        let currentRequest: CurrentRequest | null = res.stack.readBoolean() ? {
                newUpgrade: loadUpgrade(res.stack.readCell().beginParse()),
                timestamp: res.stack.readBigNumber(),
        } : null; // todo:
        let baseFiWalletCode = res.stack.readCell();
        let metadata = res.stack.readCell();
        let others = res.stack.readCell();
        return {
            supply,
            walletVersion,
            admin,
