...
export type PersonalMinterConfig = {
    supply: bigint,
    admin: Address,
...
export function personalMinterConfigCellToConfig(config: Cell): PersonalMinterConfig {
    const sc = config.beginParse()
    const parsed: PersonalMinterConfig = {
        supply: sc.loadCoins(),
        fiJettonAddr: sc.loadAddress(),
        admin: sc.loadAddress(),
        wallet_code: sc.loadRef(),
        jetton_content: sc.loadMaybeRef() instanceof Cell ? sc.loadMaybeRef()!.beginParse().loadStringTail() : null,
    };
    endParse(sc);
    return parsed;
}

export function personalMinterConfigToCell(config: PersonalMinterConfig): Cell {
...
export class PersonalMinter implements Contract {
