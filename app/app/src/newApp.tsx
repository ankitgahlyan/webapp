...
import {
  buildFiWalletActionTransaction,
  DEFAULT_FEE,
  getErrorMessage,
  isContractDeployed,
...
import { FiWalletStore } from '../../../wrappers-ts/FossFiWallet.gen';

type PendingAction = 'deploy' | 'invite' | 'vote' | 'fetch' | null;
type Theme = 'dark' | 'light';

interface JettonState {
...
const initialJettonState: JettonState = {
  status: 'idle',
  state: null,
  message: 'Connect account and fetch the current state.',
  fetchedAt: null,
};
...
export default function App() {
  ...
  const [jettonState, setJettonState] = useState<JettonState>(initialJettonState);
  const [comment, setComment] = useState('');
  const [fiJettonAddress, setFiJettonAddress] = useState(localStorage.getItem('fiJettonAddress') || '');
  ...
  async function readJettonWallet(addressValue: string) {
    const snapshot = await readFiWallet(addressValue);

    // setFiWalletAddress(snapshot.address);
    setJettonState({
  ...
  async function handleAction(action: 'invite' | 'vote') {
    if (!walletReady) {
    ...
    try {
      const transaction = buildFiWalletActionTransaction({
        action,
        addressValue: fiJettonAddress,
        fee,
        senderAddress: walletAddress!,
        comments: comment,
        jettonAmount: action === 'invite' ? '0.1' : '0.11', // todo: need?
      });

      await sendTransaction(transaction.request);
      // setCounterAddress(transaction.address);
      setStatusMessage(
        `${action === 'invite' ? 'Invite' : 'Vote'} request sent. Fetch the latest value after confirmation.`,
      );
    } catch (error) {
      setStatusMessage(getErrorMessage(error));
    } finally {
  ...
  async function handleFetch() {
    ...
    try {
      await readJettonWallet(fiJettonAddress);
  ...
  function handleNormalizeAddress() {
    ...
    try {
      setFiJettonAddress(normalizeCounterAddress(fiJettonAddress));
    } catch {
  ...
  const contractExplorerUrl = fiJettonAddress
    ? `${TONSCAN_ADDRESS_URL}/${encodeURIComponent(fiJettonAddress)}`
    : null;
        ...
        <section className="grid grid-cols-[minmax(0,1fr)_auto] gap-3 items-center mb-4 rounded-xl border bg-card px-4 py-2.5 max-md:grid-cols-1 max-sm:px-3">
          ...
          <dl className="flex items-center gap-3 text-right max-md:text-left max-sm:grid max-sm:grid-cols-2 max-sm:gap-2">
            <div className="min-w-0">
              ...
              <dd className="max-w-55 truncate text-[13px] font-medium max-sm:max-w-full">
            ...
            <CardHeader className="px-5 py-5 border-b gap-1.5">
              <div className="flex items-start justify-between gap-3">
                ...
                {walletNetworkMismatch ? (
                  <span className="rounded-full border border-(--warning)/35 px-3 py-1 text-xs font-bold text-warning">
