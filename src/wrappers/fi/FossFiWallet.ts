...
export function fossFiWalletConfigToCell(config: FossFiWalletConfig): Cell {
    return beginCell()
        ...
        .storeRef(config.timestamps) // timestamps cell (accountInit, lastInvite, lastClaim)
        .storeRef(config.addresses) // addresses cell (owner, treasury, id, nominAddrs ref, trustedJettonAddrs ref, others ref)
        .storeRef(config.maps) // other maps cell (invited, allowances, votedFor, reportInfo ref)
        .storeRef(config.baseFiWalletCode) // baseFiWalletCode
        .endCell()
}
...
export function loadTupleFiJettonFullData(source: TupleReader) {
    ...
    const _timestamps = source.readCell();
    const _addresses = source.readCell();
    const _maps = source.readCell();
...
export function loadGetterTupleAddresses(source: TupleReader) {
    ...
    // const _others = slice.loadRef().beginParse(); // todo:
    // const _a = _others.loadRef();

    return { $$type: 'Addresses' as const, owner: _owner, treasury: _treasury, id: _id, nominee: _nominee, invitor: _invitor, invitor0: _invitor0, minter: _minter, personalMinter: _personalMinter, personalJetton: _personalJetton, authorisedAccs: _authorisedAccs };
}
